...
import { GraduationCapIcon, GitHubIcon, QrCodeIcon, FingerPrintIcon } from '../constants';
...
const StudentAuthScreen: React.FC<StudentAuthScreenProps> = ({ onBack }) => {
  ...
  const handleAuthAction = async () => {
    ...
    try {
      if (isLogin) {
        ...
        sessionStorage.setItem('selectedRole', UserRole.Student);
  ...
  const handleQRScanSuccess = (scannedEmail: string) => {
    ...
    // You might want to automatically focus the password field here
  };
    ...
    <div className="flex flex-col items-center justify-center min-h-full p-4 sm:p-8 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
      <div className="w-full max-w-sm space-y-6 animate-fade-in">
        <div className="text-center space-y-4">
            ...
            <p className="text-gray-600 dark:text-gray-400">{isLogin ? 'Sign in to continue your journey.' : 'Create an account to get started.'}</p>
        </div>
